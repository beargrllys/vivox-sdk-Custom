...
/**
 * The callback interface used by applications that want to implement their own audio capture device.
 * \note Xbox 360 only.
 */
typedef struct vxa_apcd {
    /**
     * Get the user-visible name of the device.
     * @return The user-visible name of the device.
     */
    const char * (*pf_get_display_name)(struct vxa_apcd *pThis);

    /**
     * Get the internal device identifier.
    ...
    /**
     * Open the device to capture mono audio that is PCM-encoded and 16-bit interleaved at the provided sample rate.
     *
     * @param samples_per_second - The number of audio samples per second that device should handle.
     *                             It is the responsibility of the application to resample this if the
...
/**
 * The callback interface used by applications that want to implement their own audio render device.
 * \note Xbox 360 only.
 */
typedef struct vxa_aprd {
    ...
    /**
     * Open the device to render stereo audio that is PCM-encoded and 16-bit interleaved at the provided sample rate.
     *
     * @param samples_per_second - The number of audio samples per second that device should handle.
     *                             It is the responsibility of the application to resample this if the
     *                             actual hardware device does not handle the provided sample rate.
     ...
     * @param native_buffer - A place to receive the buffer handle.
     * \note This can be null. In addition, the application should only return this to pf_release_buffer.
     */
 ...
 *
 * \attention Supported on all platforms except for Xbox 360.
 *
 ...
 *
 * @param prefix - [in] The prefix for the user.
 * @param realm - [in] The realm for that user (for example, vdx5.vivox.com).
 * \ingroup diagnostics
 */
